...
export function ProfileView({ profile }: { profile: ProfileFull }) {
  return (
    <article className="mx-auto w-full max-w-2xl px-5 py-10 text-[15px] leading-relaxed text-zinc-800 sm:px-6 sm:py-16">
      <header className="border-b border-zinc-200 pb-6">
        <h1 className="text-2xl font-semibold tracking-tight text-zinc-900 break-words sm:text-[2rem]">
          {profile.display_name || profile.username}
        </h1>
        {profile.tagline && (
          <p className="mt-1.5 text-zinc-500 break-words">{profile.tagline}</p>
        )}
      </header>
